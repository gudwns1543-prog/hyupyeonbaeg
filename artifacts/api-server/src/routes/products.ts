...
const defaultProducts = [
  {
    ...
    sortOrder: 1,
    discountRate: 0,
    material: "100% 일본산 히노끼(편백) 원목",
    sizes: "900×600×600(H)mm (기본형, 맞춤제작 가능)",
    options: JSON.stringify([
      { name: "마감등급", values: ["무절", "유절"] },
      { name: "방수방식", values: ["FRP 방수", "짜맞춤"] },
    ]),
    additionalImages: "[]",
  },
  {
    ...
    discountRate: 0,
    material: "100% 일본산 히노끼(편백) 원목",
    sizes: "1200×700×650(H)mm (기본형, 맞춤제작 가능)",
    options: JSON.stringify([
      { name: "마감등급", values: ["무절", "유절"] },
      { name: "방수방식", values: ["FRP 방수", "짜맞춤"] },
    ]),
  ...
  },
  {
    ...
    material: "히노끼(편백) 원목",
    sizes: "1단 / 2단 선택",
    options: JSON.stringify([{ name: "단수", values: ["1단", "2단"] }]),
    additionalImages: "[]",
  },
  {
    name: "월풀 시스템",
...
// GET /products/admin/all — admin only, returns all including hidden
...
router.post("/", async (req, res) => {
  ...
  try {
    const {
      name, category, subCategory, price, priceText, description,
      imageUrl, additionalImages, inStock, isVisible, sortOrder,
      discountRate, material, sizes, options,
    } = req.body;
    if (!name || !category || !subCategory) {
      res.status(400).json({ error: "ValidationError", message: "상품명, 카테고리, 서브카테고리는 필수입니다" });
      return;
    }
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    if (priceText !== undefined) updates.priceText = priceText;
    ...
    if (discountRate !== undefined) updates.discountRate = Number(discountRate);
